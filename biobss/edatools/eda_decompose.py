...
def _eda_highpass(eda_signal, sr):
    ...
    tonic = nk.signal_filter(eda_signal, sampling_rate=sr,
                             highcut=0.05, method="butter")
...
def _cvxEDA(y, delta, tau0=2., tau1=0.7, delta_knot=10., alpha=8e-4, gamma=1e-2,
    ...
    i = np.arange(2, n)
    A = cv.spmatrix(np.tile(ar, (n-2, 1)),
                    np.c_[i, i, i], np.c_[i, i-1, i-2], (n, n))
    M = cv.spmatrix(np.tile(ma, (n-2, 1)),
                    np.c_[i, i, i], np.c_[i, i-1, i-2], (n, n))

    # spline
    delta_knot_s = int(round(delta_knot / delta))
    spl = np.r_[np.arange(1., delta_knot_s), np.arange(
        delta_knot_s, 0., -1.)]  # order 1
    ...
    if solver == 'conelp':
        # Use conelp
        def z(m, n): return cv.spmatrix([], [], [], (m, n))
