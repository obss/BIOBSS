...
class Process_List():
    ...
    def add_process(self, process):
        if(not process.check_modality(self.modality)):
            raise ValueError(
                'Modality of process does not match modality of process list')
        elif(not process.check_sigtype(self.sigtype)):
            raise ValueError(
                'Signal type of process does not match signal type of process list')
        else:
        ...
        for process in self.process_list:
            ...
            for c_name in c_info:
                ...
                if(isinstance(process_result, Bio_Data)):
                    ...
                    signal.add_channel(
                        process_result, c_name, modify_existed=True, modality=signal[c_name].signal_modality)
