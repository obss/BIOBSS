...
'a_cdb_a': lambda apg_sig, _0, _1, locs_a, locs_b, locs_c, locs_d, _2: (np.mean(apg_sig[locs_c]) + np.mean(apg_sig[locs_d]) - np.mean(apg_sig[locs_b]))/np.mean(apg_sig[locs_a]),
'a_bcde_a': lambda apg_sig, _0, _1, locs_a, locs_b, locs_c, locs_d, locs_e: (np.mean(apg_sig[locs_b]) - np.mean(apg_sig[locs_c]) - np.mean(apg_sig[locs_d]) - np.mean(apg_sig[locs_e]))/np.mean(apg_sig[locs_a]),
'a_bcd_a': lambda apg_sig, _0, _1, locs_a, locs_b, locs_c, locs_d, _2: (np.mean(apg_sig[locs_b]) - np.mean(apg_sig[locs_c]) - np.mean(apg_sig[locs_d]))/np.mean(apg_sig[locs_a]),
'a_be_a': lambda apg_sig, _0, _1, locs_a, locs_b, _2, _3, locs_e: (np.mean(apg_sig[locs_b]) - np.mean(apg_sig[locs_e]))/np.mean(apg_sig[locs_a]),
}
...
def get_apg_features(apg_sig: ArrayLike, locs_O:ArrayLike, fiducials:dict, sampling_rate:float, prefix: str='apg') -> dict:
    ...
    for key,func in feature_list.items():
        try:
            features["_".join([prefix, key])]=func(apg_sig, sampling_rate, locs_O, locs_a, locs_b, locs_c, locs_d, locs_e)
        except:
