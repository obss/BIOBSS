...
def get_ppg_features(sig: ArrayLike, sampling_rate:float, fiducials: dict=None, input_types: list=['cycle', 'segment'], feature_domain: dict={'cycle':['Time','Stat'], 'segment':['Stat','Freq','Time']}, prefix: str='ppg', **kwargs) -> dict:
    ...
    if 'segment' in input_types:
        ...
        features_segment = from_segment(sig, sampling_rate=sampling_rate, feature_types=feature_domain['segment'], prefix=prefix)
...
def from_segment(sig: ArrayLike,sampling_rate: float, feature_types: ArrayLike=['Stat','Freq','Time'], prefix: str='signal') -> dict:
    ...
    for domain in feature_types:
        if domain not in valid_types:
            ...
            domain_function = get_domain_function(domain)
            features.update(domain_function(sig=sig,sampling_rate=sampling_rate,input_types=['segment'],prefix=prefix))
