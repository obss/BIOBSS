...
def elim_vlf(sig: ArrayLike, sampling_rate: float) -> ArrayLike:
    ...
    filtered_signal=filtering.filter_signal(sig,filter_type,N,sampling_rate,f1,[])
...
def elim_vhf(sig: ArrayLike, sampling_rate: float) -> ArrayLike:
    ...
    Args:
        sig (Array): PPG signal
        fs (float): Sampling rate of the PPG signal
    ...
    filtered_signal=filtering.filter_signal(sig,filter_type,N,sampling_rate,[],f2)
