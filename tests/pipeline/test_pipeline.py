...
def test_m_run_process(ref_ecg_channel):
    ...
    pipeline = Bio_Pipeline()
    pipeline.set_input(ref_ecg_channel)
    pipeline.process_queue.add_process(normalize,input_signals=['ecg'],output_signals=['ecg_normalized'],sampling_rate=256)
...
def test_m_multiple_process(ref_ecg_channel):
    ...
    pipeline.set_input(ref_ecg_channel)
    pipeline.process_queue.add_process(normalize,input_signals=['ecg'],output_signals=['ecg_normalized'],sampling_rate=256)
    pipeline.process_queue.add_process(find_peaks,input_signals=['ecg_normalized'],output_signals=['ecg_peaks'],sampling_rate=256)
    pipeline.run_pipeline()
    assert True
...
def test_o_multiple_process_windowed(ref_ecg_channel):
    ...
    pipeline.set_input(ref_ecg_channel)
    pipeline.process_queue.add_process(normalize,input_signals=['ecg'],output_signals=['ecg_normalized'],sampling_rate=256)
    pipeline.process_queue.add_process(find_peaks,input_signals=['ecg_normalized'],output_signals=['ecg_peaks'],sampling_rate=256,is_event=True)
    pipeline.run_pipeline()
    assert True
...
def test_p_multiple_process_eda(sample_eda_channel):
    ...
    pipeline.process_queue.add_process(normalize,input_signals=['eda_raw'],output_signals=['eda_normalized'],sampling_rate=700)
    pipeline.process_queue.add_process(clean,input_signals=['eda_normalized'],output_signals=['eda_clean'],sampling_rate=700)
    pipeline.process_queue.add_process(decompose,input_signals=['eda_clean'],output_signals=['eda_tonic','eda_phasic'],sampling_rate=700)
    pipeline.run_pipeline()
    assert True
