...
def gold_channel_x(sample_acc):
    accx = np.asarray(sample_acc['ACCx'],)
...
def test_m_run_process(gold_channel_x):
    ...
    pipeline = Bio_Pipeline()
    pipeline.set_input(gold_channel_x)
    pipeline.process_queue.add_process(normalize,input_signals=['accx'],output_signals=['accx_normalized'],sampling_rate=32)
...
def test_nn_multiple_process(gold_channel_x):
    ...
    pipeline.set_input(gold_channel_x)
    pipeline.process_queue.add_process(normalize,input_signals=['accx'],output_signals=['accx_normalized'],sampling_rate=32)
    pipeline.process_queue.add_process(resample,input_signals=['accx_normalized'],output_signals=['accx_peaks'],sampling_rate=32,target_sampling_rate=10,new_sr=10)
    pipeline.run_pipeline()
    assert True
...
def test_o_multiple_process_windowed(gold_channel_x):
    ...
    pipeline.set_input(gold_channel_x)
    pipeline.process_queue.add_process(normalize,input_signals=['accx'],output_signals=['accx_normalized'],sampling_rate=32)
    pipeline.process_queue.add_process(resample,input_signals=['accx_normalized'],output_signals=['accx_resampled'],sampling_rate=32,target_sampling_rate=10,new_sr=10)
    pipeline.run_pipeline()
    assert pipeline.data['accx_resampled'].sampling_rate == 10 and pipeline.data['accx_resampled'].channel.shape[1] == 100
    assert True
