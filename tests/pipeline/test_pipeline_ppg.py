...
def test_m_run_process(gold_channel):
    ...
    pipeline = Bio_Pipeline()
    pipeline.set_input(gold_channel)
    pipeline.process_queue.add_process(normalize,input_signals=['ppg'],output_signals=['ppg_normalized'],sampling_rate=64)
...
def test_m_multiple_process(gold_channel):
    ...
    pipeline.set_input(gold_channel)
    pipeline.process_queue.add_process(normalize,input_signals=['ppg'],output_signals=['ppg_normalized'],sampling_rate=64)
    pipeline.process_queue.add_process(find_peaks,input_signals=['ppg_normalized'],output_signals=['ppg_peaks'],sampling_rate=64, delta=0.01)
    pipeline.run_pipeline()
    assert True
...
def test_o_multiple_process_windowed(gold_channel):
    ...
    pipeline.process_queue.add_process(normalize,input_signals=['ppg'],output_signals=['ppg_normalized'],sampling_rate=64)
    pipeline.process_queue.add_process(find_peaks,input_signals=['ppg_normalized'],output_signals=['ppg_peaks'],sampling_rate=64, delta=0.01)
    pipeline.process_queue.add_process(find_onsets,input_signals=['ppg_normalized'],output_signals=['ppg_onsets'],sampling_rate=64, delta=0.01)
    pipeline.run_pipeline()
    assert True
...
def test_p_feature_process(gold_channel):
    ...
    pipeline.set_input(gold_channel)
    pipeline.process_queue.add_process(normalize,input_signals=['ppg'],output_signals=['ppg_normalized'],sampling_rate=64)
    pipeline.process_queue.add_process(find_peaks,input_signals=['ppg_normalized'],output_signals=['ppg_peaks','ppg_onsets'],sampling_rate=64, delta=0.01,is_event=True)
    pipeline.process_queue.add_process(correct_peaks, input_signals=['ppg_normalized','ppg_peaks','ppg_onsets'],output_signals=["corrected_peaks","corrected_onsets"],sampling_rate=64,is_event=True)

    pipeline.run_pipeline()
    
    segment_features = Feature(name="segment_features", function=from_segment, feature_types=['Freq','Time','Stat'], sampling_rate=64)
    cycle_features = Feature(name="cycle_features", function=from_cycles, feature_types=['Time','Stat'], sampling_rate=64)
    pipeline.add_feature_step(segment_features, feature_prefix='ppg_segment', input_signals=['ppg_normalized'])
    pipeline.add_feature_step(cycle_features, feature_prefix='ppg_cycle', input_signals={"sig":'ppg_normalized',"peaks_locs":"corrected_peaks","troughs_locs":"corrected_onsets"})
